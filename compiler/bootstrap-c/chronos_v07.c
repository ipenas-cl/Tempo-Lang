...
typedef enum {
    ...
    AST_ARRAY_LITERAL, AST_INDEX, AST_STRUCT, AST_FIELD_ACCESS
} AstType;
...
typedef struct {
    char* name;
    int offset;
    int size;  // For arrays: number of elements (1 for scalars)
} Symbol;
...
void gen_expr(Codegen* cg, AstNode* n) {
    if (n->type == AST_NUMBER) {
        ...
        if (!strcmp(n->name, "print")) {
            ...
            // Print newline
            emit(cg, "    mov byte [rbp-32], 10\n");  // '\n'
            emit(cg, "    lea rsi, [rbp-32]\n");
            emit(cg, "    mov rdi, 1\n    mov rdx, 1\n    mov rax, 1\n    syscall\n");
        ...
        // Caller should have allocated space via symtab
        // For now, we'll handle this in AST_LET
        emit(cg, "    ; array literal\n");
        // Get variable being assigned (hack: use last symtab entry)
        if (cg->symtab && cg->symtab->count > 0) {
            ...
            // Return base address in rax
            emit(cg, "    lea rax, [rbp%d]\n", sym->offset);
        }
    } else if (n->type == AST_INDEX) {
        // Array indexing: arr[index]
        // children[0] = array (identifier)
        ...
        if (sym) {
            // Calculate index
            gen_expr(cg, n->children[1]);
            // rax = index, multiply by 8 (element size)
            emit(cg, "    imul rax, 8\n");
            // Load from [rbp + base_offset + index*8]
            emit(cg, "    mov rbx, rax\n");
...
void gen_stmt(Codegen* cg, AstNode* n) {
    if (n->type == AST_RETURN) {
        ...
        if (n->child_count > 0 && n->children[0]->type == AST_ARRAY_LITERAL) {
        ...
        if (n->child_count > 0) {
            ...
            // Array initialization handled in gen_expr
        }
...
void codegen(AstNode* ast, const char* file, StringTable* strtab) {
    ...
    // Write to file
    cg.out = fopen(file, "w");
    fprintf(cg.out, "; CHRONOS v0.5 - Stdlib Essentials\n\n");
...
int main(int argc, char** argv) {
    ...
    printf("🔥 CHRONOS v0.7 - ARRAYS\n");
    printf("Arrays + indexing enabled\n");
    printf("Compiling: %s\n", argv[1]);
